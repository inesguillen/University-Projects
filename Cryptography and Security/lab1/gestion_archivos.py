...
class GestionArchivos:
    ...
    def cargar_clave(self):
        ...
        clave_file = f"{self.__usuario}_key.bin"
        # Cargar clave existente o crear una nueva si el archivo no existe
        if os.path.exists(clave_file):
            #si existe, la abrimos en lectura binaria(para garantizar que se lea y se escriba sin ninguna alteración
            with open(clave_file, 'rb') as kf: #kf = key file
                return kf.read()  # Leer y retornar la clave existente
        else:
            nueva_clave = get_random_bytes(16)  # Generar nueva clave de 16 bytes
            with open(clave_file, 'wb') as kf:
                kf.write(nueva_clave)  # Guardar nueva clave en archivo
            return nueva_clave
    ...
    def subir_archivos(self):
        ...
        mac_generado = self.mac.generar_mac(self.key, ruta_archivo)
    ...
    def abrir_archivo(self):
        ...
        if usuario_data and pdf in usuario_data["macs"]:
            ...
            if self.mac.verificar_mac(self.key, ruta_temporal, mac_guardado):
    ...
    def cifrar_archivo(self, ruta_archivo, ruta_salida): #ruta del archivo y ruta donde se guarda el archivo cifrado
        ...
        cipher_encrypt = AES.new(self.key, AES.MODE_CFB) #crear objeto cifrador con AES en modo CFB
    ...
    def descifrar_archivo(self, ruta_archivo_encriptado, ruta_salida): #recibe la ruta del archivo y la ruta para guardar
        ...
        with open(ruta_archivo_encriptado, 'rb') as inputfile: #abrir en lectura binaria
            ...
            cipher_decrypt = AES.new(self.key, AES.MODE_CFB, iv) #crea objeto descifrador
