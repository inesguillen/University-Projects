...
class InicioSesion:
    def __init__(self, usuario):
        ...
        self.certificado = None  # Inicializar el atributo certificado

        #self.__password = password
        #no siempre será password, se recibe como variable externa
    ...
    def cargar_salt(self):
        ...
        """
        salt_file = f"{self.usuario}_salt.bin"
        if not os.path.exists(salt_file):
            print(f"Salt no encontrado para el usuario {self.usuario}. Generando uno nuevo.")
    ...
    # NUEVO ENFOQUE --> SI LLEGAMOS AQUÍ, EL USUARIO YA ESTÁ VALIDADO, COMPROBAR SÓLO CONTRASEÑA
    def iniciar_sesion_contrasena(self, password) -> bool:
        sesion_iniciada = False #inicialización de la variable
        # Saltaba un error si poníamos usuario o contrasña erróneos, pues la función trataba de devolver una
        #clave que solo se generaba si los datos eran correctos. Esto debería arreglarlo
        clave = None
        correct_password = False

        # YA NO VERIFICAMOS EL USUARIO, SOLO LO BUSCAMOS Y COMPARAMOS DATOS
        ...
        mi_usuario = datos["usuarios"].get(self.usuario)

        if mi_usuario and bcrypt.checkpw(password.encode(), mi_usuario["password_hash"].encode()):
        ...
        if not correct_password:
            ...
            clave = self.derivar_clave(password)
            #clave_privada = self.derivar_clave(password)
            #clave_publica = self.derivar_clave(password)
