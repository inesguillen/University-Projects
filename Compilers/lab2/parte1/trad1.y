...
sentencia:    IDENTIF '=' expresion      { sprintf (temp, "(setq %s %s)", $1.code, $3.code) ; 
            ...
            | '@' expresion              { sprintf (temp, "(print %s)", $2.code) ;  
                                           $$.code = gen_code (temp) ; }
            | INTEGER IDENTIF asignacion   { sprintf (temp, "(setq %s %s)", $2.code, $3.code) ;
                                            printf("Estamos leyendo int\n");
                                            $$.code = gen_code (temp) ; }
            ;

asignacion:                              { sprintf (temp, "0") ;
                                           $$.code = gen_code (temp) ;}
...
main_func:  MAIN '(' ')' '{' sentencia ';' '}'      { sprintf (temp, "(defund main()\n  %s\n)", $5.code) ;
                                           $$.code = gen_code (temp) ; }
            ;
