...
sentencia:      SETQ IDENTIF NUMBER                                         { printf("variable %s\n", $2.code);
            ...
            |   LOOP WHILE expresion DO sentencias                          { sprintf(temp, "BEGIN\n%s WHILE\n%sREPEAT", $3.code, $5.code);
                                                                              $$.code = gen_code(temp); }
            |   IF expresion '(' PROGN sentencias ')' posibleProgn          { if (strlen($7.code) > 0)
...
expresion:    termino                                   { $$ = $1 ; }
            ...
            | '(' '=' expresion expresion ')'           { sprintf(temp, "%s %s =", $3.code, $4.code);
                                                          $$.code = gen_code(temp); }
            | '(' DISTINTO expresion expresion ')'      { sprintf(temp, "%s %s <>", $3.code, $4.code);
                                                          $$.code = gen_code(temp); }
            | '(' '<' expresion expresion ')'           { sprintf(temp, "%s %s <", $3.code, $4.code);
                                                          $$.code = gen_code(temp); }
            | '(' '>' expresion expresion ')'           { sprintf(temp, "%s %s >", $3.code, $4.code);
                                                          $$.code = gen_code(temp); }
            | '(' MAYOR_IGUAL expresion expresion ')'   { sprintf(temp, "%s %s >=", $3.code, $4.code);
                                                          $$.code = gen_code(temp); }
            | '(' MENOR_IGUAL expresion expresion ')'   { sprintf(temp, "%s %s <=", $3.code, $4.code);
                                                          $$.code = gen_code(temp); }
